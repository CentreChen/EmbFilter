...
from models.modeling_qwen2_lr import Qwen2ForCausalLM
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filter_ratio", type=int, default=2)
    parser.add_argument("--filter_type", type=str, default="edge")
    ...
    model = Qwen2ForCausalLM.from_pretrained(pretrained_model, torch_dtype=torch.bfloat16, config=config)

    filter_num = config.hidden_size // args.filter_ratio
    model.set_lm_embed(filter_num = filter_num, filter_type = args.filter_type)
    print(f"dim = {filter_num}")
    model = model.model
    ...
    for task_type, tasks in ALL_TASKS:
        ...
        results = evaluation.run(
            model, 
            eval_splits=["test"], 
            output_folder=os.path.join("./output", f"{args.model_name}_prompteol_{filter_num}_{args.filter_type}", task_type),
            verbosity=0,
...
# CUDA_VISIBLE_DEVICES=0 python run4qwen_prompteol.py --filter_ratio 1